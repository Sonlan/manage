...
import org.biacbiac.manage.service.AgentService;
import org.biacbiac.manage.utils.*;
...
//        String s1 = MD5Util.encode(key);
    ...
    public void regact(@RequestParam String key,@RequestParam String mail,HttpServletRequest request,HttpServletResponse response) throws Exception{
        response.setContentType("application/json;charset=utf-8");
        switch (agentService.regact(key,mail)){
            case 0:{ //激活成功
                response.setContentType("image/jpg");
                byte [] image = QRCode.encode("https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxa76ca32f306d7277&redirect_uri=http://1u5186s163.51mypc.cn/manage/test/tojson&response_type=code&scope=snsapi_userinfo&state=a13#wechat_redirect",250,250,"jpg");
